...
There has been a lot of work on the exploitation of UGC and social media for telling events' stories.

The focus spans from collecting and mining resources to assembling them in new ways to represent a story. For example, the first topic is treated in \cite{becker:WSDM12}, where the authors provide techniques for automatically identifying contents shared by users on social platforms for apriori known events, and describe different querying techniques. In \cite{naaman:CSCW12} the focus is shifted towards identifying the users contributing to the social content available on a particular happening.
